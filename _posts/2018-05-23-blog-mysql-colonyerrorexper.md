...
## 当主数据库挂掉后还原
读写分离的master数据库因不明原因挂起后还原

### 准备材料
一台作为访问测试的机子 ubuntu16

三台配置好主从的centos7（一主多从） 
master:
```text
...
+-----------+---------------+------+-----------+--------------------------------------+
|         3 | 172.16.47.136 | 3306 |         1 | c2e1598d-5981-11e8-a10c-000c2986eda0 |
|         2 | 172.16.47.132 | 3306 |         1 | d256497b-58e5-11e8-b36c-000c2991d93f |
+-----------+---------------+------+-----------+--------------------------------------+
...
录入成功------当前录入ID4   耗时：0.008 秒。1527240124

录入成功------当前录入ID5   耗时：0.007 秒。1527240125

录入成功------当前录入ID6   耗时：0.025 秒。1527240126
```

随便打开一台从服务器(我这里打开server_id = 2,172.16.47.132)
```text
mysql> select * from gek_identifi_company;
...
| 69 | 5b07d5fe532ff | 9   | 1527240190 |
+----+---------------+-----+------------+
69 rows in set (0.00 sec)
```

已经在正常运行了！
这时，我模拟主服务器(我这里打开server_id = 1,172.16.47.134) 不明原因挂掉了.
![Image text](/assets/images/blogs/mysql-colonyerrorexper/vwarestop.jpg)
直接关机了

查看从数据库与ubuntu记录的p.log
```text
#p.log
录入成功------当前录入ID621   耗时：0.014 秒。1527240835

录入成功------当前录入ID622   耗时：0.006 秒。1527240836
链接失败
链接失败
链接失败
链接失败
...
#server_id 2
 | 621 | 5b07d883af7b2 | 9   | 1527240835 |
 | 622 | 5b07d884b3a79 | 9   | 1527240836 |
 +-----+---------------+-----+------------+
...
### 并发高的情况
```text
root@ubuntu:~# ab -n500 -c100 http://172.16.47.129/Exper/mysql/persion.php?rn=1
```
在执行中，模拟server1 关掉 ，然后重启，结果出错误了



```text
p.log
录入成功------当前录入ID2863   耗时：0.012 秒。1527304575
录入成功------当前录入ID2864   耗时：0.012 秒。1527304576
```



## 不知道啥问题，slave pos 点错误
### 问题的生产方法
环境
![Image text](/assets/images/blogs/mysql-colonyerrorexper/c1.jpg)
